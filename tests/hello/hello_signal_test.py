...
from hello.hello_signal import (
    GreetingWorkflow
)
...
async def test_signal_workflow(client: Client):
    ...
    async with Worker(
        ...
        wf_execution: WorkflowExecutionDescription = await handle.describe();
        assert WorkflowExecutionStatus.COMPLETED == wf_execution.status